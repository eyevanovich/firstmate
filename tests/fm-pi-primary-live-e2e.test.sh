...
# adapter and script surfaces under verification.
cp -R "$ROOT/.pi/." "$PROJECT/.pi/"
cp -R "$ROOT/bin/." "$PROJECT/bin/"
mkdir -p "$HOME_DIR/state" "$HOME_DIR/config"

"$TMUX" -L "$SOCKET" new-session -d -s "$SESSION" -c "$PROJECT" \
  "bash -lc 'export FM_HOME=\"$HOME_DIR\" FM_ROOT_OVERRIDE=\"$PROJECT\" FM_POLL=1 FM_SIGNAL_GRACE=0 FM_HEARTBEAT=600; printf \"%s\\n\" \"\$\$\" > \"\$FM_HOME/state/.lock\"; pi --approve --no-session --no-context-files --no-extensions -e .pi/extensions/fm-primary-pi-watch.ts --model openai-codex/gpt-5.6-sol --thinking low; rc=\$?; printf \"PI_EXIT=%s\\n\" \"\$rc\"; sleep 300'"

i=0
while [ "$i" -lt 120 ]; do
  [ -f "$HOME_DIR/state/.pi-watch-extension-loaded" ] && break
  sleep 0.5
  i=$((i + 1))
done
