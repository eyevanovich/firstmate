...
test_primary_and_secondmate_instruction_generation() {
  ...
  assert_grep 'reads `AGENTS.md` fresh on launch' "$SECONDMATE" \
    "new secondmate sessions do not load tracked authority"
  assert_grep "running-home instruction-surface advance needs one" "$SECONDMATE" \
    "running secondmates are not nudged after tracked authority advances"
  ...
  pass "primary workers and secondmates receive the authority rule through normal owners"
}
