...
test_pi_extension_forces_followup() {
  ...
  assert_contains "$content" 'fm_primary_scope_matches' "pi extension loaded marker must delegate worker-copy isolation to the shared primary-scope owner"
...
test_hook_claude_mode_malformed_rewake_epoch_blocks() {
  local dir out status
  dir=$(make_primary_dir "$TMP_ROOT/hook-claude-malformed-epoch")
  : > "$dir/state/task1.meta"
  printf 'prefix epoch=3 owner_pid=999 outcome=rewake updated_at=%s suffix\n' "$(date +%s)" > "$dir/state/.claude-autoarm-epoch"
  out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=200 run_hook_claude "$dir" true); status=$?
  expect_code 2 "$status" "--claude mode must reject a malformed private rewake record even when freshly written"
  pass "fm-turnend-guard --claude: malformed rewake epoch does not allow a blind stop"
}

test_hook_claude_mode_touched_stale_rewake_epoch_blocks() {
  local dir out status
  dir=$(make_primary_dir "$TMP_ROOT/hook-claude-touched-stale-epoch")
  : > "$dir/state/task1.meta"
  printf 'epoch=3 owner_pid=999 outcome=rewake updated_at=1\n' > "$dir/state/.claude-autoarm-epoch"
  touch "$dir/state/.claude-autoarm-epoch"
  out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=200 run_hook_claude "$dir" true); status=$?
  expect_code 2 "$status" "--claude mode must reject stale private epoch content even when its file mtime is fresh"
  pass "fm-turnend-guard --claude: stale rewake content cannot be revived by touching the private record"
}
