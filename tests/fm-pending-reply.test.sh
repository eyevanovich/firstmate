...
#  12. Private records reject malformed, foreign, linked, and wrong-device data
#  13. Unsafe delivery and wrong-home artifacts are never followed or removed
set -u
...
test_escalation_publication_failure_retries() {
  local home state corr target escalations
...
test_wrong_home_detected_not_acknowledged() {
  ...
  mark_secondmate_home "$sm_home" hibit
...
test_tick_skips_terminal_and_reuses_target_observation() {
  (
    ...
    mark_secondmate_home "$home/escalated" escalated
...
test_private_record_integrity_is_mandatory() {
  local home state corr rec before after victim alias foreign tmp actual_uid actual_device
  home=$(setup_parent private-records)
  state="$home/state"
  export FM_PENDING_REPLY_NOW=9500

  corr=$(fm_pending_reply_create "$home" "$state" hibit malformed)
  rec=$(fm_pending_reply_path "$state" "$corr")
  printf 'foreign_key=value\n' >> "$rec"
  fm_pending_reply_record_valid_path "$rec" 2>/dev/null \
    && fail "unknown record keys must be rejected"
  before=$(cat "$rec")
  fm_pending_reply_tick "$state" || fail "tick should skip malformed records"
  after=$(cat "$rec")
  [ "$after" = "$before" ] || fail "tick must not mutate malformed records"
  fm_pending_reply_discard_undelivered "$state" "$corr" 2>/dev/null \
    && fail "cleanup must refuse malformed records"
  [ -f "$rec" ] || fail "malformed record must remain for explicit recovery"

  corr=$(fm_pending_reply_create "$home" "$state" hibit symlink)
  rec=$(fm_pending_reply_path "$state" "$corr")
  victim="$state/symlink-victim"
  mv "$rec" "$victim"
  ln -s "$victim" "$rec"
  fm_pending_reply_record_valid_path "$rec" 2>/dev/null \
    && fail "symlink records must be rejected"
  fm_pending_reply_discard_undelivered "$state" "$corr" 2>/dev/null \
    && fail "cleanup must refuse symlink records"
  [ -L "$rec" ] && [ -f "$victim" ] || fail "symlink cleanup must not remove either artifact"

  corr=$(fm_pending_reply_create "$home" "$state" hibit hardlink)
  rec=$(fm_pending_reply_path "$state" "$corr")
  alias="$state/hardlink-alias"
  ln "$rec" "$alias"
  fm_pending_reply_record_valid_path "$rec" 2>/dev/null \
    && fail "multiply linked records must be rejected"
  fm_pending_reply_discard_undelivered "$state" "$corr" 2>/dev/null \
    && fail "cleanup must refuse multiply linked records"
  [ -f "$rec" ] && [ -f "$alias" ] || fail "hard-link cleanup must preserve both names"

  corr=$(fm_pending_reply_create "$home" "$state" hibit wronghome)
  rec=$(fm_pending_reply_path "$state" "$corr")
  foreign="$TMP_ROOT/foreign-parent-$RANDOM"
  mkdir -p "$foreign/state"
  tmp="$rec.rewrite"
  awk -v value="$foreign" '
    /^parent_home=/ { print "parent_home=" value; next }
    { print }
  ' "$rec" > "$tmp"
  chmod 0600 "$tmp"
  mv "$tmp" "$rec"
  fm_pending_reply_record_valid_path "$rec" 2>/dev/null \
    && fail "records naming a different parent home must be rejected"

  corr=$(fm_pending_reply_create "$home" "$state" hibit ownership)
  rec=$(fm_pending_reply_path "$state" "$corr")
  actual_uid=$(fm_pending_reply_file_uid "$state")
  (
    fm_pending_reply_file_uid() {
      [ "$1" = "$rec" ] && { printf '999999'; return 0; }
      printf '%s' "$actual_uid"
    }
    fm_pending_reply_record_valid_path "$rec" 2>/dev/null \
      && fail "foreign-owned records must be rejected"
  )
  chmod 0644 "$rec"
  fm_pending_reply_record_valid_path "$rec" 2>/dev/null \
    && fail "non-private record modes must be rejected"
  chmod 0600 "$rec"
  actual_device=$(fm_pending_reply_file_device "$state")
  (
    fm_pending_reply_file_device() {
      [ "$1" = "$rec" ] && { printf 'foreign-device'; return 0; }
      printf '%s' "$actual_device"
    }
    fm_pending_reply_record_valid_path "$rec" 2>/dev/null \
      && fail "wrong-device records must be rejected"
    fm_pending_reply_discard_undelivered "$state" "$corr" 2>/dev/null \
      && fail "cleanup must refuse wrong-device records"
  )
  [ -f "$rec" ] || fail "wrong-device cleanup must preserve the record"
  pass "private pending records reject malformed and foreign artifacts"
}

test_unsafe_artifacts_are_not_followed_or_cleaned() {
  local home state corr rec marker victim sm_home external hits alias
  home=$(setup_parent unsafe-artifacts)
  state="$home/state"
  export FM_PENDING_REPLY_NOW=9600

  corr=$(fm_pending_reply_create "$home" "$state" hibit delivery)
  rec=$(fm_pending_reply_path "$state" "$corr")
  marker=$(fm_pending_reply_delivery_confirmation_path "$state" "$corr")
  victim="$state/delivery-victim"
  printf 'attempted=9600\n' > "$victim"
  ln -s "$victim" "$marker"
  fm_pending_reply_prepare_delivery "$state" "$corr" 2>/dev/null \
    && fail "delivery preparation must refuse a symlink marker"
  fm_pending_reply_discard_undelivered "$state" "$corr" 2>/dev/null \
    && fail "cleanup must refuse a symlink marker"
  [ -L "$marker" ] && [ -f "$rec" ] || fail "unsafe delivery artifacts must remain untouched"
  [ "$(cat "$victim")" = attempted=9600 ] || fail "delivery marker target must not be modified"

  corr=$(fm_pending_reply_create "$home" "$state" hibit parentstatus)
  fm_pending_reply_mark_delivered "$state" "$corr"
  external="$state/external-status"
  printf 'done [corr=%s]: untrusted alias\n' "$corr" > "$external"
  ln -s "$external" "$state/hibit.status"
  fm_pending_reply_try_resolve "$state" "$corr" 2>/dev/null \
    && fail "parent resolution must not follow a symlink status"
  [ "$(phase_of "$state" "$corr")" = awaiting_report ] \
    || fail "symlink parent status must not resolve the record"

  sm_home="$TMP_ROOT/unsafe-sm-$RANDOM"
  mkdir -p "$sm_home/state"
  printf 'hibit\n' > "$sm_home-marker-target"
  ln -s "$sm_home-marker-target" "$sm_home/.fm-secondmate-home"
  printf 'done [corr=%s]: wrong home\n' "$corr" > "$sm_home/state/hibit.status"
  fm_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" \
    || fail "unsafe wrong-home marker should be ignored"
  rec=$(fm_pending_reply_path "$state" "$corr")
  hits=$(fm_pending_reply_get "$rec" wrong_home_hits)
  [ "$hits" = 0 ] || fail "symlink home markers must not authorize a wrong-home scan"
  rm "$sm_home/.fm-secondmate-home"
  printf 'wrong-task\n' > "$sm_home/.fm-secondmate-home"
  fm_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" \
    || fail "mismatched wrong-home marker should be ignored"
  hits=$(fm_pending_reply_get "$rec" wrong_home_hits)
  [ "$hits" = 0 ] || fail "mismatched home identities must not count reports"
  printf 'hibit\n' > "$sm_home/.fm-secondmate-home"
  alias="$sm_home/state/status-hardlink"
  ln "$sm_home/state/hibit.status" "$alias"
  fm_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" \
    || fail "hard-linked wrong-home status should be skipped"
  hits=$(fm_pending_reply_get "$rec" wrong_home_hits)
  [ "$hits" = 0 ] || fail "multiply linked status files must not count reports"
  rm "$alias"
  printf 'done [corr=%s]: owned report\n' "$corr" >> "$sm_home/state/hibit.status"
  fm_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" \
    || fail "owned wrong-home status should be detected"
  hits=$(fm_pending_reply_get "$rec" wrong_home_hits)
  [ "$hits" = 2 ] || fail "both owned correlated lines should count after validation, got $hits"
  pass "unsafe delivery, parent-status, and wrong-home artifacts are never followed"
}

test_private_directory_symlink_is_refused() {
  local home state foreign before after
  home=$(setup_parent private-dir-symlink)
  state="$home/state"
  foreign="$TMP_ROOT/private-dir-target-$RANDOM"
  mkdir -p "$foreign"
  chmod 0755 "$foreign"
  ln -s "$foreign" "$state/pending-replies"
  before=$(fm_pending_reply_file_mode "$foreign")
  fm_pending_reply_create "$home" "$state" hibit unsafe 2>/dev/null \
    && fail "create must refuse a symlink pending directory"
  after=$(fm_pending_reply_file_mode "$foreign")
  [ "$after" = "$before" ] || fail "refused symlink directories must not be chmodded"
  [ -z "$(find "$foreign" -mindepth 1 -maxdepth 1 -print -quit)" ] \
    || fail "refused symlink directories must remain empty"
  pass "private pending directory symlinks are refused without side effects"
}
