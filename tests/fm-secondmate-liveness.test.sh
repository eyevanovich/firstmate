...
test_agent_state_dispatcher_and_compatibility() {
  ...
  for backend in zellij orca cmux; do
    out=$(bash -c '. "$0/bin/fm-backend.sh"; fm_backend_agent_state "$1" target' "$ROOT" "$backend")
    [ "$out" = unverified ] || fail "$backend should remain unverified, got '$out'"
    out=$(bash -c '. "$0/bin/fm-backend.sh"; fm_backend_agent_alive "$1" target' "$ROOT" "$backend")
    [ "$out" = unknown ] || fail "the compatibility dispatcher should map $backend's unverified result to unknown, got '$out'"
  done

  pass "fm_backend_agent_state: routes tmux/Herdr and preserves Zellij/Orca/cmux unverified semantics"
}
