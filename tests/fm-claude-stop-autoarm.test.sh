...
test_inert_with_malformed_session_lock() {
  local dir out status
  dir=$(make_primary_dir "$TMP_ROOT/malformed-session-lock")
  : > "$dir/state/task.meta"
  printf '123\n456\n' > "$dir/state/.lock"
  write_arm_fixture "$dir" actionable
  out=$(printf '%s\n' '{"session_id":"malformed"}' | FM_HOME="$dir" "$FAKE_CLAUDE" -c '"$FM_HOME/bin/fm-claude-stop-autoarm.sh"' 2>&1); status=$?
  expect_code 0 "$status" "a malformed session-lock record must remain inert rather than be reclaimed"
  [ "$(cat "$dir/state/.lock")" = $'123\n456' ] || fail "malformed session-lock record was replaced"
  [ ! -e "$dir/state/arm-ran" ] || fail "hook armed from a malformed session-lock record"
  [ ! -e "$dir/state/.claude-autoarm-epoch" ] || fail "hook wrote an epoch from a malformed session-lock record"
  pass "auto-arm: malformed session-lock records remain inert"
}
...
test_private_owner_lock_handles_malformed_and_stale_records() {
  local fresh_dir stale_dir out status
  fresh_dir=$(make_primary_dir "$TMP_ROOT/malformed-owner-fresh")
  : > "$fresh_dir/state/task.meta"
  mkdir -p "$fresh_dir/state/.claude-autoarm.lock"
  printf 'not-a-pid\n' > "$fresh_dir/state/.claude-autoarm.lock/pid"
  write_arm_fixture "$fresh_dir" actionable
  out=$(run_autoarm "$fresh_dir" 2>/dev/null); status=$?
  expect_code 0 "$status" "a fresh malformed private owner record must not be stolen"
  [ ! -e "$fresh_dir/state/arm-ran" ] || fail "hook armed through a fresh malformed private owner record"

  stale_dir=$(make_primary_dir "$TMP_ROOT/malformed-owner-stale")
  : > "$stale_dir/state/task.meta"
  mkdir -p "$stale_dir/state/.claude-autoarm.lock"
  printf 'not-a-pid\n' > "$stale_dir/state/.claude-autoarm.lock/pid"
  touch -t 202001010000 "$stale_dir/state/.claude-autoarm.lock"
  write_arm_fixture "$stale_dir" actionable
  out=$(run_autoarm "$stale_dir" 2>/dev/null); status=$?
  expect_code 2 "$status" "an aged malformed private owner record must be reclaimed for an actionable cycle"
  [ -e "$stale_dir/state/arm-ran" ] || fail "hook did not reclaim the aged malformed private owner record"
  [ ! -e "$stale_dir/state/.claude-autoarm.lock" ] || fail "reclaimed private owner lock survived cycle cleanup"
  pass "auto-arm: fresh malformed private ownership is preserved while aged malformed ownership is reclaimed"
}
...
test_nonworker_private_records_do_not_create_supervision_need() {
  local dir out status
  dir=$(make_primary_dir "$TMP_ROOT/nonworker-records")
  printf 'window=observer-only\n' > "$dir/state/task.observer"
  printf '%s\n' "$$" > "$dir/state/.supervise-daemon.pid"
  printf 'gitlab diagnostic\n' > "$dir/state/tool-diagnostic"
  printf 'unrelated-backend-window\n' > "$dir/state/backend-window"
  write_arm_fixture "$dir" actionable
  out=$(run_autoarm "$dir" 2>/dev/null); status=$?
  expect_code 0 "$status" "observer, daemon, diagnostic, and unrelated backend records must not create supervision work"
  [ ! -e "$dir/state/arm-ran" ] || fail "hook counted a nonworker private record as an in-flight worker"
  [ ! -e "$dir/state/.claude-autoarm-epoch" ] || fail "hook claimed a cycle for nonworker private records"
  pass "auto-arm: observer, daemon, diagnostic, and unrelated backend records are not workers or supervision owners"
}
...
test_two_successive_stop_cycles_share_one_session_owner() {
  local dir rc1 rc2 count epoch
  dir=$(make_primary_dir "$TMP_ROOT/two-cycles")
  : > "$dir/state/task.meta"
  write_arm_fixture "$dir" actionable
  FM_HOME="$dir" "$FAKE_CLAUDE" -c '
    printf "%s\n" "$$" > "$FM_HOME/state/.lock"
    printf "%s\n" "{\"session_id\":\"s\"}" | "$FM_HOME/bin/fm-claude-stop-autoarm.sh" >/dev/null 2>/dev/null
    echo $? > "$FM_HOME/state/rc1"
    printf "%s\n" "{\"session_id\":\"s\"}" | "$FM_HOME/bin/fm-claude-stop-autoarm.sh" >/dev/null 2>/dev/null
    echo $? > "$FM_HOME/state/rc2"
  '
  rc1=$(cat "$dir/state/rc1")
  rc2=$(cat "$dir/state/rc2")
  count=$(wc -l < "$dir/state/arm-ran" | tr -d ' ')
  epoch=$(sed -n 's/^epoch=\([0-9][0-9]*\) .*/\1/p' "$dir/state/.claude-autoarm-epoch")
  [ "$rc1" = 2 ] && [ "$rc2" = 2 ] || fail "two successive Stop cycles must each translate one actionable close, got rc1=$rc1 rc2=$rc2"
  [ "$count" -eq 2 ] || fail "two successive Stop cycles must foreground exactly two arms, saw $count"
  [ "$epoch" = 4 ] || fail "two successive Stop cycles must record one claim and one outcome per cycle, got epoch $epoch"
  [ ! -e "$dir/state/.claude-autoarm.lock" ] || fail "owner lock survived two completed Stop cycles"
  pass "auto-arm: two complete Stop-owned cycles use one session owner without overlap"
}
