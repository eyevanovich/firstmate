...
phase_send() {
  ...
  assert_grep 'send-keys -t firstmate:fm-design -l ' "$LOG" "send did not use the window recorded in this home's meta"
  assert_grep 'FIRSTMATE_OP: v1 from-firstmate:' "$LOG" "send did not type the canonical secondmate request"
