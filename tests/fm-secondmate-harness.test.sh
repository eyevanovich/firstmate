...
test_config_push_propagates_reports_without_ff_or_nudge() {
  ...
  assert_contains "$(cat "$log")" "FIRSTMATE_OP: v1 from-firstmate:" \
    "config reread must use the canonical marked routed secondmate path"
...
test_config_reread_per_home_changed_sets_and_exact_bytes() {
  ...
  assert_contains "$(cat "$log")" "FIRSTMATE_OP: v1 from-firstmate:" "reread send must use the canonical marker"
...
test_config_reread_bootstrap_path_and_spawn_flexibility() {
  ...
  assert_contains "$(cat "$log")" "FIRSTMATE_OP: v1 from-firstmate:" \
    "bootstrap config reread must use the canonical routed secondmate send"
  assert_contains "$(cat "$instr")" \
    $'-----BEGIN config/crew-harness-----\ncodex\n-----END config/crew-harness-----' \
