...
The same launch and the same locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/forge-hosts`, `config/signing-agent`, and the one shared captain-preference file `data/captain-shared.md`.
...
No AGENTS.md reread nudge is needed at spawn or respawn because the agent reads `AGENTS.md` fresh on launch along with its other instructions; only the bootstrap sweep's running-home instruction-surface advance needs one, namely that AGENTS.md re-read.
