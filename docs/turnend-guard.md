...
The epoch proof accepts only the hook owner's exact single-line record shape and requires both its `updated_at` value and file age to be within `FM_CLAUDE_AUTOARM_EPOCH_FRESH`; malformed content, stale content with a touched mtime, and future timestamps never authorize a stop.
...
`tests/fm-turnend-guard.test.sh` covers the predicate, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the cooperative `--claude` claim wait, exact/fresh epoch allow, malformed and stale epoch refusal, re-block budget, Pi logical-run latching, missing-`jq` behavior, all five registrations, and Grok resume permission and recursion safety.
`tests/fm-supervision-instructions.test.sh` covers recovery-line ownership.
`FM_PI_LIVE_E2E=1 tests/fm-pi-primary-live-e2e.test.sh` is the opt-in isolated Pi path.
[`watcher-continuity.md`](watcher-continuity.md#current-live-evidence-2026-07-28-pdt--2026-07-29-utc) records current live evidence and the Claude authentication blocker.
