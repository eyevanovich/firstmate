...
- **No verified agent-process liveness classifier.** The session-start secondmate liveness sweep therefore receives `unverified` from `fm_backend_agent_state` for zellij and reports that the recovery classifier is unverified instead of killing or respawning the endpoint.
