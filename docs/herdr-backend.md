...
At the same time, an independent no-mistakes run-step attribution fallback could miss this crew's branch when `axi status` reported another branch; current `bin/fm-crew-state.sh` falls back to top-level `no-mistakes runs --limit ${FM_CREW_STATE_RUNS_LIMIT:-200}` and accepts only a row bound to the crew's branch and current code identity.
...
The cross-branch attribution fallback now uses the real `no-mistakes runs` command with the same exact head-history binding as direct status attribution, and the watcher checks provably-working evidence before a stale status-log verb can make a stale pane terminal.
...
Classification policy, batching, the max-defer escape, locks, and wake-queue handling were unchanged by the Herdr transport work.
The later typed-input migration replaced the old `FM_INJECT_MARK` construction with the canonical `away-supervisor` envelope owned by `bin/fm-operational-input.sh`; the historical bytes documented above remain classifier-only compatibility.
