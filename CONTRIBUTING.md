...
Firstmate's wrapper still matters: implementation workers route every `ask-user` finding to firstmate, which applies the authority procedure in `.agents/skills/ask-user-authority/SKILL.md`; crewmates avoid `--yes` because it would bypass that check and any required captain escalation.
