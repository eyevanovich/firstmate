...
function safeOperationalInputScript(): string {
  const stat = lstatSync(configuredScript);
  if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1 || (stat.mode & 0o111) === 0) {
    throw new Error(`unsafe operational-input encoder: ${configuredScript}`);
  }
...
function runOperationalInputCommand(
  ...
  const result = spawnSync(safeOperationalInputScript(), args, {
    encoding: "utf8",
    input: content,
    maxBuffer: 1024 * 1024,
  });
