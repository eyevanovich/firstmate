...
#
# The epoch ledger state/.claude-autoarm-epoch records the latest claim and outcome.
# Its exact single-line shape is `epoch=<N> owner_pid=<N> outcome=<arming|afk|clean|rewake> updated_at=<epoch-seconds>`.
# The synchronous Stop guard (bin/fm-turnend-guard.sh --claude) accepts only that exact fresh record.
# This lets it allow a stop whose recovery this hook already owns without forcing a duplicate continuation for the same event epoch.
#
