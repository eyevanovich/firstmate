...
signed_commit_smoke() {
  local repo=$1 format=$2 key=$3 program=$4 failure=$5 scratch commit
...
preflight() {
  ...
  if [ "$format" = ssh ]; then
    ...
    if signing_agent_configured; then
      ...
      signed_commit_smoke "$repo" "$format" "$public" "$SELF" \
        "configured signing agent cannot create a signed isolated commit"
  ...
  signed_commit_smoke "$repo" "$format" "$key" '' \
    "configured signing identity cannot create a signed isolated commit"
}
