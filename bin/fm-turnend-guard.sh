...
autoarm_owns_recovery() {
  ...
  if [[ "$record" =~ ^epoch=([0-9]{1,20})\ owner_pid=([0-9]{1,20})\ outcome=(arming|afk|clean|rewake)\ updated_at=([0-9]{1,12})$ ]]; then
    ...
    updated_at=${BASH_REMATCH[4]}
  else
    return 1
  fi
  if [ "$outcome" = rewake ]; then
    path_age=$(fm_path_age "$STATE/.claude-autoarm-epoch")
    now=$(date +%s)
    [ "$updated_at" -le "$now" ] 2>/dev/null || return 1
    record_age=$((now - updated_at))
    [ "$path_age" -lt "$EPOCH_FRESH" ] && [ "$record_age" -lt "$EPOCH_FRESH" ] && return 0
  fi
  return 1
}
