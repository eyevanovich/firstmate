...
#
# Record integrity and lifecycle:
#   - pending-replies/ is an owner-only mode-0700 ordinary directory under the
#     effective parent state directory (`FM_HOME/state` or the explicit
#     `FM_STATE_OVERRIDE`); each record and delivery marker is a mode-0600
#     ordinary single-link file on that directory's device.
#   - a record is accepted only when its filename, corr_id, schema, exact key
#     set, task id, canonical parent home, and exact parent status destination
#     agree. Invalid records are inert and never authorize delivery, recovery,
#     resolution, escalation, or cleanup of another path.
#   - records are created before delivery, remain durable through one recovery
#     and one escalation, become terminal only at resolved, and are never
#     inferred from or reconciled by reading secondmate chat.
#
...
FM_PENDING_REPLY_RECORD_KEYS='schema corr_id task_id parent_home parent_status parent_status_scan_signature request_summary created_epoch delivered_epoch phase turn_seen_busy request_turn_completed_epoch recovery_attempted_epoch recovery_sender_pid recovery_sender_identity recovery_sent_epoch recovery_delivery_outcome recovery_turn_seen_busy recovery_turn_completed_epoch escalated_epoch resolved_epoch resolved_via wrong_home_hits wrong_home_sightings wrong_home_scan_signature grace_secs'

fm_pending_reply_file_mode() {
...
fm_pending_reply_private_dir_safe() {  # <dir> <owner-parent-dir>
  local dir=$1 owner=$2 dir_abs owner_abs mode
  [ -d "$dir" ] && [ ! -L "$dir" ] || return 1
  [ -d "$owner" ] && [ ! -L "$owner" ] || return 1
  dir_abs=$(cd "$dir" 2>/dev/null && pwd -P) || return 1
  owner_abs=$(cd "$owner" 2>/dev/null && pwd -P) || return 1
  [ "${dir_abs%/*}" = "$owner_abs" ] || return 1
  mode=$(fm_pending_reply_file_mode "$dir") || return 1
  [ "$mode" = 700 ] || return 1
  [ "$(fm_pending_reply_file_device "$dir")" = "$(fm_pending_reply_file_device "$owner")" ] || return 1
  [ "$(fm_pending_reply_file_uid "$dir")" = "$(id -u)" ] || return 1
}

fm_pending_reply_private_file_safe() {  # <file> <owner-dir>
  local file=$1 owner=$2 file_abs owner_abs
  [ -f "$file" ] && [ ! -L "$file" ] || return 1
  [ -d "$owner" ] && [ ! -L "$owner" ] || return 1
  file_abs=$(cd "${file%/*}" 2>/dev/null && pwd -P) || return 1
  owner_abs=$(cd "$owner" 2>/dev/null && pwd -P) || return 1
  [ "$file_abs" = "$owner_abs" ] || return 1
  [ "$(fm_pending_reply_file_links "$file")" = 1 ] || return 1
  [ "$(fm_pending_reply_file_mode "$file")" = 600 ] || return 1
  [ "$(fm_pending_reply_file_device "$file")" = "$(fm_pending_reply_file_device "$owner")" ] || return 1
  [ "$(fm_pending_reply_file_uid "$file")" = "$(id -u)" ] || return 1
}

fm_pending_reply_corr_valid() {
  printf '%s' "$1" | grep -Eq '^[a-f0-9]{16}$'
}

fm_pending_reply_record_keys_valid() {  # <record>
  local rec=$1
  awk -F= -v expected="$FM_PENDING_REPLY_RECORD_KEYS" '
    BEGIN {
      count = split(expected, keys, " ")
      for (i = 1; i <= count; i++) want[keys[i]] = 1
    }
    {
      key = $1
      if (!(key in want) || ++seen[key] != 1) bad = 1
    }
    END {
      for (key in want) if (seen[key] != 1) bad = 1
      exit bad
    }
  ' "$rec" 2>/dev/null
}

fm_pending_reply_value_direct() {  # <record> <key>
  awk -F= -v key="$2" '$1 == key { sub(/^[^=]*=/, ""); print; exit }' "$1" 2>/dev/null
}

fm_pending_reply_epoch_or_empty() {
  case "$1" in ''|*[!0-9]*) [ -z "$1" ] ;; *) return 0 ;; esac
}

fm_pending_reply_record_valid_path() {  # <record>
  local rec=$1 dir state corr task parent_home parent_home_abs parent_status state_abs state_override_abs expected_status phase value key
  dir=${rec%/*}
  state=${dir%/*}
  fm_pending_reply_private_dir_safe "$dir" "$state" || return 1
  fm_pending_reply_private_file_safe "$rec" "$dir" || return 1
  fm_pending_reply_record_keys_valid "$rec" || return 1
  corr=${rec##*/}
  fm_pending_reply_corr_valid "$corr" || return 1
  [ "$(fm_pending_reply_value_direct "$rec" schema)" = "$FM_PENDING_REPLY_SCHEMA" ] || return 1
  [ "$(fm_pending_reply_value_direct "$rec" corr_id)" = "$corr" ] || return 1
  task=$(fm_pending_reply_value_direct "$rec" task_id)
  case "$task" in ''|*[!A-Za-z0-9._-]*|*/*) return 1 ;; esac
  parent_home=$(fm_pending_reply_value_direct "$rec" parent_home)
  case "$parent_home" in /*) ;; *) return 1 ;; esac
  [ -d "$parent_home" ] && [ ! -L "$parent_home" ] || return 1
  parent_home_abs=$(cd "$parent_home" 2>/dev/null && pwd -P) || return 1
  [ -d "$state" ] && [ ! -L "$state" ] || return 1
  state_abs=$(cd "$state" 2>/dev/null && pwd -P) || return 1
  if [ "$state" != "$parent_home/state" ] || [ "$state_abs" != "$parent_home_abs/state" ]; then
    [ -n "${FM_STATE_OVERRIDE:-}" ] && [ -d "$FM_STATE_OVERRIDE" ] \
      && [ ! -L "$FM_STATE_OVERRIDE" ] || return 1
    state_override_abs=$(cd "$FM_STATE_OVERRIDE" 2>/dev/null && pwd -P) || return 1
    [ "$state" = "$FM_STATE_OVERRIDE" ] && [ "$state_abs" = "$state_override_abs" ] || return 1
  fi
  parent_status=$(fm_pending_reply_value_direct "$rec" parent_status)
  expected_status="$state/$task.status"
  [ "$parent_status" = "$expected_status" ] || return 1
  phase=$(fm_pending_reply_value_direct "$rec" phase)
  case "$phase" in awaiting_report|delivery_unknown|recovery_sending|recovery_sent|recovery_failed|recovery_unknown|escalated|resolved) ;; *) return 1 ;; esac
  for key in created_epoch delivered_epoch request_turn_completed_epoch recovery_attempted_epoch recovery_sent_epoch recovery_turn_completed_epoch escalated_epoch resolved_epoch grace_secs; do
    value=$(fm_pending_reply_value_direct "$rec" "$key")
    fm_pending_reply_epoch_or_empty "$value" || return 1
  done
  [ -n "$(fm_pending_reply_value_direct "$rec" created_epoch)" ] || return 1
  [ -n "$(fm_pending_reply_value_direct "$rec" grace_secs)" ] || return 1
  for key in turn_seen_busy recovery_turn_seen_busy; do
    case "$(fm_pending_reply_value_direct "$rec" "$key")" in 0|1) ;; *) return 1 ;; esac
  done
  value=$(fm_pending_reply_value_direct "$rec" recovery_sender_pid)
  fm_pending_reply_epoch_or_empty "$value" || return 1
  value=$(fm_pending_reply_value_direct "$rec" wrong_home_hits)
  case "$value" in ''|*[!0-9]*) return 1 ;; esac
  case "$(fm_pending_reply_value_direct "$rec" recovery_delivery_outcome)" in ''|confirmed|failed|unknown) ;; *) return 1 ;; esac
  case "$(fm_pending_reply_value_direct "$rec" resolved_via)" in ''|status|document|helper) ;; *) return 1 ;; esac
  return 0
}

fm_pending_reply_delivery_marker_valid() {  # <state-dir> <corr-id> <marker>
  local state=$1 corr=$2 marker=$3 dir line kind epoch
  dir=$(fm_pending_reply_dir "$state")
  fm_pending_reply_private_file_safe "$marker" "$dir" || return 1
  [ "${marker##*/}" = ".delivery-confirmed-$corr" ] || return 1
  line=$(cat "$marker" 2>/dev/null) || return 1
  [ "$(printf '%s\n' "$line" | wc -l | tr -d ' ')" = 1 ] || return 1
  kind=${line%%=*}
  epoch=${line#*=}
  case "$kind" in attempted|confirmed) ;; *) return 1 ;; esac
...
fm_pending_reply_get() {  # <record-path> <key>
  local rec=$1 key=$2
  fm_pending_reply_record_valid_path "$rec" || return 0
  fm_pending_reply_value_direct "$rec" "$key"
}
...
fm_pending_reply_corr_reusable() {  # <state-dir> <corr_id> <task_id>
  ...
  fm_pending_reply_record_valid_path "$rec" || return 1
...
fm_pending_reply_set() {  # <record-path> <key> <value>
  ...
  fm_pending_reply_record_valid_path "$rec" || return 1
  case " $FM_PENDING_REPLY_RECORD_KEYS " in *" $key "*) ;; *) return 1 ;; esac
  case "$value" in *$'\n'*|*$'\r'*) return 1 ;; esac
  dir=${rec%/*}
  base=${rec##*/}
  tmp=$(umask 077; mktemp "$dir/.${base}.tmp.XXXXXX" 2>/dev/null) || return 1
  ...
  fm_pending_reply_record_valid_path "$rec"
}
...
fm_pending_reply_create() {  # <parent-home> <state-dir> <task_id> <request-text>
  ...
  local dir rec corr now summary status_path tmp parent_home_abs state_abs state_override_abs
  ...
  [ -d "$state" ] && [ ! -L "$state" ] || return 1
  state_abs=$(cd "$state" 2>/dev/null && pwd -P) || return 1
  if [ "$state_abs" != "$parent_home_abs/state" ]; then
    [ -n "${FM_STATE_OVERRIDE:-}" ] && [ "$state" = "$FM_STATE_OVERRIDE" ] \
      && [ -d "$FM_STATE_OVERRIDE" ] && [ ! -L "$FM_STATE_OVERRIDE" ] || return 1
    state_override_abs=$(cd "$FM_STATE_OVERRIDE" 2>/dev/null && pwd -P) || return 1
    [ "$state_abs" = "$state_override_abs" ] || return 1
  fi
  dir=$(fm_pending_reply_dir "$state")
  if [ ! -e "$dir" ] && [ ! -L "$dir" ]; then
...
EOF
  chmod 0600 "$tmp" 2>/dev/null || { rm -f "$tmp"; return 1; }
  mv -f "$tmp" "$rec" || { rm -f "$tmp"; return 1; }
  fm_pending_reply_record_valid_path "$rec" || { rm -f "$rec"; return 1; }
  printf '%s' "$corr"
}
...
fm_pending_reply_write_delivery_confirmation() {  # <state-dir> <corr_id> <state> <value>
  ...
  dir=${marker%/*}
  fm_pending_reply_private_dir_safe "$dir" "$pending_state" || return 1
  if [ -e "$marker" ] || [ -L "$marker" ]; then
    fm_pending_reply_delivery_marker_valid "$pending_state" "$corr" "$marker" || return 1
  fi
  tmp=$(umask 077; mktemp "$dir/.delivery-confirmed-${corr}.tmp.XXXXXX" 2>/dev/null) || return 1
  printf '%s=%s\n' "$delivery_state" "$value" > "$tmp" || { rm -f "$tmp"; return 1; }
  chmod 0600 "$tmp" 2>/dev/null || { rm -f "$tmp"; return 1; }
  mv -f "$tmp" "$marker" || { rm -f "$tmp"; return 1; }
  fm_pending_reply_delivery_marker_valid "$pending_state" "$corr" "$marker"
}
...
fm_pending_reply_prepare_delivery() {  # <state-dir> <corr_id>
  ...
  if [ -e "$marker" ] || [ -L "$marker" ]; then
    fm_pending_reply_delivery_marker_valid "$state" "$corr" "$marker" || return 1
    return 0
  fi
  now=$(fm_pending_reply_now)
...
fm_pending_reply_confirm_delivery() {  # <state-dir> <corr_id>
  ...
  if fm_pending_reply_mark_delivered "$state" "$corr" "$now"; then
    fm_pending_reply_delivery_marker_valid "$state" "$corr" "$marker" || return 2
    rm -f "$marker" 2>/dev/null || return 2
    return 0
  fi
  return 2
}
...
fm_pending_reply_find_resolve_line() {  # <status-file> <corr_id>
  local status_file=$1 corr=$2 line owner
  [ -f "$status_file" ] && [ ! -L "$status_file" ] || return 0
  [ "$(fm_pending_reply_file_links "$status_file")" = 1 ] || return 0
  owner=${status_file%/*}
  [ -d "$owner" ] && [ ! -L "$owner" ] || return 0
  [ "$(fm_pending_reply_file_device "$status_file")" = "$(fm_pending_reply_file_device "$owner")" ] || return 0
  while IFS= read -r line || [ -n "$line" ]; do
    [ -n "$line" ] || continue
...
fm_pending_reply_file_signature() {  # <path>
  local path=$1 owner
  [ -f "$path" ] && [ ! -L "$path" ] || { printf 'missing'; return 0; }
  [ "$(fm_pending_reply_file_links "$path")" = 1 ] || { printf 'unsafe'; return 0; }
  owner=${path%/*}
  [ -d "$owner" ] && [ ! -L "$owner" ] || { printf 'unsafe'; return 0; }
  [ "$(fm_pending_reply_file_device "$path")" = "$(fm_pending_reply_file_device "$owner")" ] || { printf 'unsafe'; return 0; }
  if [ "$(uname -s 2>/dev/null)" = Darwin ]; then
...
fm_pending_reply_status_set_signature() {  # <status-dir>
  ...
  [ -d "$status_dir" ] && [ ! -L "$status_dir" ] || { printf 'unsafe'; return 0; }
  {
    for status_file in "$status_dir"/*.status; do
      [ -f "$status_file" ] && [ ! -L "$status_file" ] || continue
      [ "$(fm_pending_reply_file_links "$status_file")" = 1 ] || continue
      [ "$(fm_pending_reply_file_device "$status_file")" = "$(fm_pending_reply_file_device "$status_dir")" ] || continue
...
fm_pending_reply_try_resolve() {  # <state-dir> <corr_id> [status-file-override]
  ...
  if [ -n "$status_override" ] && [ "$status_override" != "$status_file" ]; then
    return 1
  fi
...
fm_pending_reply_maybe_escalate() {  # <state-dir> <corr_id>
  ...
  [ -d "$state" ] && [ ! -L "$state" ] || return 1
  [ "${parent_status%/*}" = "$state" ] || return 1
  if [ -e "$parent_status" ] || [ -L "$parent_status" ]; then
    [ -f "$parent_status" ] && [ ! -L "$parent_status" ] || return 1
    [ "$(fm_pending_reply_file_links "$parent_status")" = 1 ] || return 1
    [ "$(fm_pending_reply_file_device "$parent_status")" = "$(fm_pending_reply_file_device "$state")" ] || return 1
  fi
...
fm_pending_reply_detect_wrong_home() {  # <state-dir> <corr_id> <secondmate-home>
  ...
  local task_id marker sm_home_abs parent_home parent_home_abs
  rec=$(fm_pending_reply_path "$state" "$corr")
  fm_pending_reply_record_valid_path "$rec" || return 1
  [ -n "$sm_home" ] && [ -d "$sm_home" ] && [ ! -L "$sm_home" ] || return 0
  sm_home_abs=$(cd "$sm_home" 2>/dev/null && pwd -P) || return 0
  parent_home=$(fm_pending_reply_get "$rec" parent_home)
  parent_home_abs=$(cd "$parent_home" 2>/dev/null && pwd -P) || return 0
  [ "$sm_home_abs" != "$parent_home_abs" ] || return 0
  sm_home=$sm_home_abs
  task_id=$(fm_pending_reply_get "$rec" task_id)
  marker="$sm_home/.fm-secondmate-home"
  [ -f "$marker" ] && [ ! -L "$marker" ] || return 0
  [ "$(fm_pending_reply_file_links "$marker")" = 1 ] || return 0
  [ "$(fm_pending_reply_file_device "$marker")" = "$(fm_pending_reply_file_device "$sm_home_abs")" ] || return 0
  [ "$(tr -d '[:space:]' < "$marker" 2>/dev/null)" = "$task_id" ] || return 0
  [ -d "$sm_home_abs/state" ] && [ ! -L "$sm_home_abs/state" ] || return 0
  phase=$(fm_pending_reply_get "$rec" phase)
  ...
  for status_file in "$sm_home"/state/*.status; do
    [ -f "$status_file" ] && [ ! -L "$status_file" ] || continue
    [ "$(fm_pending_reply_file_links "$status_file")" = 1 ] || continue
    [ "$(fm_pending_reply_file_device "$status_file")" = "$(fm_pending_reply_file_device "$sm_home/state")" ] || continue
    line_no=0
    while IFS= read -r line || [ -n "$line" ]; do
      line_no=$((line_no + 1))
...
fm_pending_reply_tick() {  # <state-dir>
  ...
  dir=$(fm_pending_reply_dir "$state")
  [ -d "$dir" ] || return 0
  fm_pending_reply_private_dir_safe "$dir" "$state" || return 0
  for rec in "$dir"/*; do
    fm_pending_reply_record_valid_path "$rec" || continue
    corr=${rec##*/}
    task_id=$(fm_pending_reply_get "$rec" task_id)
    phase=$(fm_pending_reply_get "$rec" phase)
    ...
    if [ "$phase" = escalated ]; then
      ...
      if [ -f "$meta" ] && [ ! -L "$meta" ] \
        && [ "$(fm_pending_reply_file_links "$meta")" = 1 ] \
        && [ "$(fm_pending_reply_file_device "$meta")" = "$(fm_pending_reply_file_device "$state")" ]; then
